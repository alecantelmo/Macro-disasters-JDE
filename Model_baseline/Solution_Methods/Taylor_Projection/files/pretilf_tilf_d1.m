...
full_rows=zeros(3,1);
full_cols=zeros(3,1);
full_vals=zeros(n_s,3);
compressed_deriv=zeros(n_s,3);
compressed_deriv(:,1)=u05.*exp(logtilkstarbackp);
compressed_deriv(:,2)=u05.*exp(logtilk).*(DELTA - 1);
compressed_deriv(:,3)=exp(logtilkstarbackp) + exp(logtilk).*(DELTA - 1);
uncompressed_deriv=compressed_deriv(:,index.nnz{1,1});
full_vals(:,1:3)=uncompressed_deriv;
full_cols(1:3,:)=index.loc{1,1};
full_rows(1:3)=1;
derivs=sptensor(full_rows,full_cols,full_vals,1,repmat(18,1,1));
